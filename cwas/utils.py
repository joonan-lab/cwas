...
def bgzip_tabix(in_file_path: str, force_overwrite: int = 0):
    ...
    if not force_overwrite and os.path.isfile(gz_path):
        print(f'[{get_curr_time()}, Progress] A bgzipped file for "{in_file_path}" already exists so skip this step')
    else:
        print(f'[{get_curr_time()}, Progress] bgzip and tabix for "{in_file_path}"')
