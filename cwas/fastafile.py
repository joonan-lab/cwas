...
class FastaFile:
    def __init__(self, fasta_file_path: str):
        ...
        :param fasta_file_path: This file path should end with
                                '.fa' or '.fa.gz'.
        """
        if not fasta_file_path.endswith(
            ".fa.gz"
        ...
        fasta_idx_path = (
            fasta_file_path.replace(".gz", ".fai")
            if is_gzip
        ...
        if not os.path.isfile(fasta_idx_path):
            raise FileNotFoundError(
                "A fai index file of the input fasta file "
    ...
    def get_base(self, chrom: str, pos: int) -> str:
        ...
        if pos < 0 or pos >= self._idx_info_dict[chrom]["size"]:
            raise ValueError(
                "The input position should be in the range "
                "[0, chromosome size)."
            )
    ...
    def get_seq(self, chrom: str, start: int, end: int) -> str:
        ...
        if start >= end:
            raise ValueError(
                "The end position must be larger " "than the start position."
            )

        if start < 0 or end > self._idx_info_dict[chrom]["size"]:
            raise ValueError(
                "The sequence should be in the range " "[0, chromosome size]."
            )
    ...
    def _conv_pos_to_idx(self, chrom: str, pos: int) -> int:
        ...
        return (
            ...
            + pos
            // self._idx_info_dict[chrom]["seq_len"]
