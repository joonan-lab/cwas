...
class Configuration(Runnable):
    ...
    def _check_args_validity(args: argparse.Namespace):
        ...
        def _check_is_dir(dirpath: Path):
            if not dirpath.is_dir():
                raise NotADirectoryError(
                    f'"{dirpath}" is not a directory or ' f"invalid."
                )
        ...
        if args.work_dir.exists():
            if args.work_dir.is_dir():
                log.print_warn(
                    f'The CWAS workspace "{args.work_dir}" already ' f"exists."
                )
            else:
                raise NotADirectoryError(
                    f'The CWAS workspace "{args.work_dir}"'
    ...
    def _set_env(self):
        ...
        self.set_env("CWAS_WORKSPACE", getattr(self, "work_dir"))
