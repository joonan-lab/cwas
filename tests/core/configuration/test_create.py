...
def test_split_annotation_key(cwas_workspace, annotation_key_conf):
    ...
    with bed_key_conf.open() as f:
        bed_key = yaml.safe_load(f)
    assert (
        "bed_annot1.bed.gz" in bed_key
        and bed_key["bed_annot1.bed.gz"] == "bed1"
    )
    assert (
        "bed.annot2.bed.gz" in bed_key
        and bed_key["bed.annot2.bed.gz"] == "bed2"
    )
...
def test_create_bw_cutoff_list(cwas_workspace, annotation_dir, bw_cutoff_conf):
    ...
    for bw_filename, bw_key in bw_key_dict.items():
        assert (
            bw_key in bw_cutoff_dict
            and user_def_cutoff_dict[bw_filename] == bw_cutoff_dict[bw_key]
        )
...
def test_create_category_domain_list(
    ...
    create.create_category_domain_list(
        domain_list_path, bed_key_conf, bw_key_conf, gene_matrix
    )
    ...
    with domain_list_path.open("r") as domain_list_f, bed_key_conf.open(
        "r"
    ) as bed_key_f, bw_key_conf.open("r") as bw_key_f, gene_matrix.open(
        "r"
    ) as gene_mat_f:
